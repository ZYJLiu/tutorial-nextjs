...
In this lesson, we'll learn to build a transaction to transfer SOL from one wallet to another.

Before we get started, let's go over the concepts of transactions and instructions on Solana.

## Transactions Overview

To invoke programs from the client, we build and send transactions to be processed by the network.

Let’s expand on the details that make up a transaction:

![](/course/1-transaction.png)
...
![](/course/2-transaction.png)

Instructions in a transaction are processed sequentially, in the order they are added to a transaction.
The network ensures that transactions are processed such that all instructions either complete successfully or, if one fails, the entire transaction is rejected.

## Instructions Overview

Next, let’s expand on the details that make up a instruction. Each instruction must specify 3 pieces of information:
...
In the following sections, we'll learn to build a transaction to transfer SOL using the `@solana/web3.js` library.
