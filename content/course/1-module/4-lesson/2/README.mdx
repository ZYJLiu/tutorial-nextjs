...
## Step 1: Create Token Account

A transaction to create a token account requires two instructions:

1. Instruction to invoke the System Program to create a new account with the appropriate size and lamports required for a token account.
2. Instruction to invoke the Token Program to initialize the token account data
...
First, calculate the minimum amount of lamports required to store the data for a token account using the `getMinimumBalanceForRentExemptMint` function from `@solana/spl-token`.
...
The `ACCOUNT_SIZE` constant from `@solana/spl-token` specifies the bytes required for the token account data.
